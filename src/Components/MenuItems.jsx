...
function MenuItems({id,title,len ,subItems,showDrop,handleSubMenuItems}) {

  return (
     <div className='w-full border-amber-500  px-1 border-t-1' key={id}>
    <button className='w-full pt-1 pb-2 px-1 cursor-pointer' onClick={handleSubMenuItems}>
     
        <div className='w-full flex justify-between'>
         <h1 className='text-xl text-start font-semibold text-amber-500 '>{title+" ("+len+")"}</h1>
         <span  className='cursor-pointer px-1 py-1'>
           {showDrop?<FaChevronUp className='text-amber-500' />: <FaChevronDown className='text-amber-500' />}
         </span>
        </div>
        {/* sub items */ }
       {showDrop  &&<div className=' flex flex-col gap-4 mt-4'> {subItems.map((detail)=><div key={detail?.card?.info?.id}><SubMenuItems 
